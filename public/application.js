...
/***/ (function(module, exports, __webpack_require__) {
	...
	var playlistEvents = __webpack_require__(13);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onCreatePlaylist = function onCreatePlaylist(event) {
	  ...
	  api.createPlaylist(data).then(ui.createPlaylistSuccess).catch(ui.createPlaylistFailure);
	  // } else {
	  //   console.log('Please provide a playlist name!')
	  // }
	};

	var onGetPlaylists = function onGetPlaylists(event) {
	  event.preventDefault();

	  api.indexPlaylists().then(ui.indexPlaylistSuccess).catch(ui.indexPlaylistFailure);
	};

	var onGetPlaylist = function onGetPlaylist(event) {
	  event.preventDefault();
	  var playlist = getFormFields(event.target).playlist;

	  if (playlist.id.length !== 0) {
	    api.showPlaylist(playlist.id).then(ui.showPlaylistSuccess).catch(ui.showPlaylistFailure);
	  } else {
	    console.log('Please provide a playlist id!');
	  }
	};

	var onUpdatePlaylist = function onUpdatePlaylist(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);

	  if (data.playlist.id.length !== 0) {
	    api.updatePlaylist(data).then(ui.updatePlaylistSuccess).catch(ui.updatePlaylistFailure);
	  } else {
	    console.log('Please provide a playlist id!');
	  }
	};
	...
	var onDeletePlaylist = function onDeletePlaylist(event) {
	  ...
	  if (playlist.id.length !== 0) {
	    api.destroyPlaylist(playlist.id).then(ui.deletePlaylistSuccess).catch(ui.deletePlaylistFailure);
	  } else {
	    console.log('Please provide a playlist id!');
	  }
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var indexPlaylists = function indexPlaylists(data) {
	  return $.ajax({
	    url: config.apiOrigin + '/playlists',
	    method: 'GET',
	    headers: {
	...
	var updatePlaylist = function updatePlaylist(data) {
	  return $.ajax({
	...
	var destroyPlaylist = function destroyPlaylist(data) {
	  return $.ajax({
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(11);
	var showPlaylistsTemplate = __webpack_require__(16);
	...
	var createPlaylistFailure = function createPlaylistFailure() {
	  $('input').val('');
	  // $('#newPlaylistModal').modal('hide')
	  $('.message').text('Error creating playlist!');
	};

	var indexPlaylistSuccess = function indexPlaylistSuccess(data) {
	  store.playlists = data.playlists;
	  var showPlaylistsHtml = showPlaylistsTemplate({
	...
	var indexPlaylistFailure = function indexPlaylistFailure() {
	  $('.message').text('Error showing playlists!');
	};

	var showPlaylistSuccess = function showPlaylistSuccess(data) {
	  console.log(data, 'this is data');
	...
	var showPlaylistFailure = function showPlaylistFailure() {
	  $('.message').text('Errors!');
	};

	var updatePlaylistSuccess = function updatePlaylistSuccess(response) {
	  $('input').val('');
	  $('.message').text('Updated!');
	};

	var updatePlaylistFailure = function updatePlaylistFailure(response) {
	  // $('.message').text('Error updating playlist!')
	  $('input').val('');
	  $('.message').text('Error updating playlist!');
	};

	var deletePlaylistSuccess = function deletePlaylistSuccess(response) {
	  $('input').val('');
	  $('.message').text('Deleted!');
	};

	var deletePlaylistFailure = function deletePlaylistFailure(response) {
	  $('.message').text('Error deleting playlist!');
	};

	module.exports = {
	  createPlaylistSuccess: createPlaylistSuccess,
	  createPlaylistFailure: createPlaylistFailure,
	  indexPlaylistSuccess: indexPlaylistSuccess,
	  indexPlaylistFailure: indexPlaylistFailure,
	  showPlaylistSuccess: showPlaylistSuccess,
	  showPlaylistFailure: showPlaylistFailure,
	  updatePlaylistSuccess: updatePlaylistSuccess,
	  updatePlaylistFailure: updatePlaylistFailure,
	  deletePlaylistSuccess: deletePlaylistSuccess,
	  deletePlaylistFailure: deletePlaylistFailure
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "<table id=\"playlist-table\" class=\"table\">\n\n  <thead>\n    <tr>\n      <th>#</th>\n      <th>Playlist Title</th>\n    </tr>\n  </thead>\n\n  <tbody id=\"playlist-table-body\">\n"
...
/***/ (function(module, exports, __webpack_require__) {
	...
	exports.push([module.id, "@import url(https://fonts.googleapis.com/css?family=Raleway:500);", ""]);
